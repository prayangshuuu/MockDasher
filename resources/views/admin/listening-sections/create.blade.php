        ...
        <form action="{{ route('admin.listening-sections.store', $test->id) }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-sm border border-gray-200 rounded-lg p-8">
            ...
            <div class="mb-6">
                ...
                <select name="section_number" class="w-full md:w-1/2 border-gray-300 rounded-md shadow-sm focus:ring-purple-500 focus:border-purple-500 sm:text-sm">
                    <option value="1">Section 1 (Conversation, everyday context)</option>
                    <option value="2">Section 2 (Monologue, everyday context)</option>
                    <option value="3">Section 3 (Conversation, educational context)</option>
                    <option value="4">Section 4 (Monologue, academic subject)</option>
                </select>
            </div>

            <div class="mb-8 border border-dashed border-gray-300 rounded-lg p-6 bg-gray-50 text-center">
                <label class="block text-gray-700 text-sm font-semibold mb-3">Audio Recording (Required)</label>
                ...
                <p class="text-xs text-gray-400 mt-2">Max 10MB limit</p>
            </div>

            <div class="mb-6 border border-gray-100 p-4 rounded bg-gray-50">
                <label class="block text-gray-700 text-sm font-semibold mb-2">Transcript / Passage Text (Optional)</label>
                <p class="text-xs text-gray-500 mb-2">Useful for admin reference or specific question setups.</p>
