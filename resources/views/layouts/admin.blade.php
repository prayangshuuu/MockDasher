...
>
    ...
    <nav class="flex-1 space-y-1 overflow-y-auto px-4 py-6">
        ...
        @foreach($navItems as $item)
            @php $isActive = Route::has($item['route']) ? request()->routeIs($item['match']) : false; @endphp
    ...
    <div class="border-t border-slate-200 dark:border-slate-800 px-6 py-5 bg-slate-50/50 dark:bg-slate-900/20">
        <div class="flex items-center gap-3">
            ...
            <div class="min-w-0">
                ...
                <p class="truncate text-[11px] font-black uppercase tracking-wider text-primary mt-0.5">Administrator</p>
            </div>
        </div>
    </div>
</aside>
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="min-w-0">
                @hasSection('breadcrumbs')
                    @yield('breadcrumbs')
                @else
                    <h1 class="truncate text-lg font-bold tracking-tight text-slate-900 dark:text-white">@yield('title', 'Admin Dashboard')</h1>
                @endif
            </div>
        </div>
        ...
        <div x-data="{ open: false }" class="relative flex items-center gap-3">
            <button @click="open = !open" @click.outside="open = false" class="flex items-center gap-3 rounded-full px-2 py-1.5 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800 border border-transparent hover:border-slate-200 dark:hover:border-slate-700">
                ...
                <div class="hidden text-left sm:block">
                    ...
                    <p class="mt-1 text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">Admin</p>
                </div>
