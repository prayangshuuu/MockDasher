...
<head>
    ...
    <title>{{ config('app.name', 'MockDasher') }} — @yield('title', 'Admin Dashboard')</title>
...
>
    ...
    <div class="flex items-center gap-3 px-6 py-6">
        ...
        <div class="min-w-0">
            <h1 class="text-lg font-bold tracking-tight text-[var(--color-text-primary)] leading-none">MockDasher</h1>
            <p class="text-[10px] font-bold text-[var(--color-primary)] uppercase tracking-wider mt-1">Admin Panel</p>
        </div>
        {{-- Mobile close --}}
    ...
    <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-2">
        <p class="mb-2 px-3 text-[10px] font-bold uppercase tracking-widest text-[var(--color-text-secondary)]">Menu</p>
        ...
        @foreach($navItems as $item)
            @php 
                $isActive = Route::has($item['route']) ? request()->routeIs($item['match']) : false; 
            @endphp
    ...
    <div class="border-t border-[var(--color-divider)] px-4 py-4">
        <div class="flex items-center gap-3">
            ...
            <div class="min-w-0">
                ...
                <p class="truncate text-xs text-[var(--color-primary)] font-medium">Administrator</p>
            </div>
        </div>
    </div>
</aside>
...
<div class="flex min-w-0 flex-1 flex-col">
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="min-w-0">
                @hasSection('breadcrumbs')
                    @yield('breadcrumbs')
                @else
                    <h1 class="truncate text-lg font-bold tracking-tight text-[var(--color-text-primary)]">@yield('title', 'Admin Dashboard')</h1>
                @endif
            </div>
        </div>
        ...
        <div x-data="{ open: false }" class="relative flex items-center gap-3">
            ...
            >
                ...
                <div class="hidden text-left sm:block">
                    ...
                    <p class="mt-0.5 text-xs text-[var(--color-text-secondary)]">Admin</p>
                </div>
            ...
            <div
                ...
                class="absolute right-0 top-full mt-2 w-52 rounded-[var(--radius-lg)] border border-[var(--color-divider)] bg-[var(--color-bg-primary)] py-1 shadow-[var(--shadow-premium)]"
            >
    ...
    <footer class="border-t border-[var(--color-divider)] px-4 py-6 text-center text-xs text-[var(--color-text-secondary)] sm:px-6 lg:px-8">
        &copy; {{ date('Y') }} {{ config('app.name', 'MockDasher') }} Admin. All rights reserved.
    </footer>
</div>
