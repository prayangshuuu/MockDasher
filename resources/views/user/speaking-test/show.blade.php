...
@section('timer_area')
<div x-data="speakingTimer({{ $remainingSeconds }})" class="flex items-center gap-3 bg-white dark:bg-slate-800 border-2 px-4 py-2 rounded-xl shadow-sm transition-all" :class="timeRemaining <= 60 ? 'border-rose-500 bg-rose-50 dark:bg-rose-950/20' : 'border-slate-100 dark:border-slate-700'">
...
<div class="flex items-center gap-6">
    ...
    <button @click="$dispatch('trigger-submit')" class="flex items-center gap-2 px-5 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-black rounded-lg text-xs font-black uppercase tracking-widest hover:scale-105 transition-all active:scale-95 shadow-lg shadow-slate-200 dark:shadow-none">
...
<div x-data="speakingApp('{{ route('user.speaking.submit', $attempt->id) }}')"
    ...
    <div class="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center px-12 gap-12 z-10 shrink-0">
        @foreach($parts as $partNumber => $questions)
            <div class="flex items-center gap-4 transition-all" :class="currentPart == {{ $partNumber }} ? 'opacity-100' : 'opacity-30'">
                <div class="size-8 rounded-lg flex items-center justify-center text-xs font-black transition-all"
                     :class="currentPart == {{ $partNumber }} ? 'exam-gradient text-white shadow-lg' : (currentPart > {{ $partNumber }} ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-400')">
                    {{ $partNumber }}
                </div>
                <span class="text-[10px] font-black uppercase tracking-widest" :class="currentPart == {{ $partNumber }} ? 'text-primary' : 'text-slate-400'">Part {{ $partNumber }}</span>
            </div>
            @if(!$loop->last)
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar p-12">
        <div class="max-w-4xl mx-auto flex flex-col items-center text-center">
            @foreach($parts as $partNumber => $questions)
                <div x-show="currentPart == {{ $partNumber }}" x-cloak class="w-full space-y-12 animate-in fade-in zoom-in-95 duration-500">
                    ...
                    <div class="space-y-4">
                        ...
                        <h2 class="text-5xl font-black text-slate-900 dark:text-white tracking-tight leading-tight">
                            {{ $partNumber == 2 ? 'Long Turn Topic' : ($partNumber == 1 ? 'Introduction & Interview' : 'Discussion') }}
                        </h2>
                    </div>

                    <!-- Prompts / Questions -->
                    <div class="grid grid-cols-1 gap-6 w-full text-left">
                        @foreach($questions as $pi => $question)
                            <div class="p-8 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border-2 border-slate-100 dark:border-slate-800 transition-all hover:border-primary/20 shadow-soft">
                    ...
                    <div class="pt-12 flex items-center justify-center gap-8">
                        ...
                        @if($partNumber < count($parts))
                            <button @click="currentPart++; stopRecording();" class="px-10 py-5 bg-primary text-white rounded-2xl text-sm font-black uppercase tracking-[0.2em] shadow-xl shadow-primary/20 hover:scale-105 transition-all">
                                Next Part
                            </button>
                        @else
                            <button @click="submitInterview()" class="px-10 py-5 bg-emerald-600 text-white rounded-2xl text-sm font-black uppercase tracking-[0.2em] shadow-xl shadow-emerald-500/20 hover:scale-105 transition-all">
                                End Interview
                            </button>
                        @endif
                    </div>
    ...
    <form id="speaking-form" :action="submitUrl" method="POST" enctype="multipart/form-data" class="hidden">
        ...
        <textarea name="transcript" x-model="transcript" class="hidden"></textarea>
    </form>
</div>
@endsection
...
<script>
    ...
    function speakingApp(submitUrl) {
        return {
            currentPart: {{ $parts->keys()->first() ?? 1 }},
            transcript: '',
            recognition: null,
            submitUrl: submitUrl,
